...
it("should transform special characters", () => {
  ...
  expect("aguañón".split("").map(transformToConsumableChar).join("")).toBe(
    "aguanon"
  );
  expect("aguará".split("").map(transformToConsumableChar).join("")).toBe(
    "aguara"
  );
  expect("propósito".split("").map(transformToConsumableChar).join("")).toBe(
    "proposito"
  );
});
